...
async function loadNotesList() {
  ...
  try {
    ...
    if (notes.length === 0) {
      notesList.innerHTML =
        "<p>No notes on this page yet. Right-click anywhere on the page to create one!</p>";
      return;
    }
    ...
    notesList.innerHTML =
      '<p class="error">Error loading notes. Please try again.</p>';
  }
}
...
// Button navigation
document
  .querySelector(".landing .btn-primary")
  .addEventListener("click", () => showScreen("onboarding-1"));
document
  .querySelector(".onboarding-1 .btn-primary")
  .addEventListener("click", () => showScreen("onboarding-2"));
document
  .querySelector(".onboarding-2 .btn-primary")
...
navLinks.forEach((link) => {
  link.addEventListener("click", (e) => {
    e.preventDefault();
    let targetScreen = link.getAttribute('location');
    if(targetScreen === null || targetScreen.trim().length < 1){
      targetScreen = link.textContent.toLowerCase().replace(" ", "-");
    }
...
// Menu buttons
document
  .querySelector(".menu .btn:nth-child(2)")
  ?.addEventListener("click", () => showScreen("main-screen")); // Notes
document
  .querySelector(".menu .btn:last-child")
  .addEventListener("click", () => {
